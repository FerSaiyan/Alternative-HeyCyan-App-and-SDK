...
export default async function CheckoutPage({ searchParams }: CheckoutPageProps) {
  ...
  if (!userId) {
    redirect("/signin?auth=required&next=/single_purchase_onboarding");
  }
        ...
        <section className="mx-auto max-w-3xl">
          <GlassCard className="glass-card-strong p-6 sm:p-8">
            ...
            <SectionHeading
              ...
              subtitle="Pedimos só o essencial para personalizar a experiência do seu familiar idoso. Em seguida, você finaliza a compra dos óculos."
              className="animate-in"
            />

            <OnboardingFormWrapper>
              <form className="mt-6 space-y-4" action="/coupon" method="get">
                {checkoutQuestions.map((item, index) => (
