      ...
      <main className="container-width pt-8 sm:pt-12">
        <section className="mx-auto grid max-w-5xl gap-5 lg:grid-cols-[1.1fr,0.9fr]">
          <GlassCard className="glass-card-strong p-6 sm:p-7">
            <p className="pill-eyebrow">Pagamento seguro</p>
            <h1 className="mt-3 text-3xl font-semibold tracking-tight text-slate-900">
              {purchaseType === "one_time" ? "Finalize a compra do óculos" : "Finalize sua assinatura mensal"}
            </h1>
            <p className="mt-2 text-sm text-muted">
              {IS_ASAAS
                ? purchaseType === "one_time"
                  ? "Óculos inteligente CareLens: pagamento único de R$ 250, via PIX, boleto ou cartão (parcelado em até 6x)."
                  : "Assinatura mensal CareLens: R$ 50/mês para serviços de IA, via cartão de crédito. Cancele após 3 meses."
                : "Você pode pagar com cartão ou carteira digital (Google Pay / Apple Pay, quando disponível)."}
            </p>
          ...
          <GlassCard className="p-6 sm:p-7">
            <h2 className="text-lg font-semibold text-slate-900">Privacidade de pagamento</h2>
            <ul className="mt-4 space-y-2 text-sm text-slate-800">
              {IS_ASAAS ? (
                <>
                  <li>- O pagamento é processado via Asaas (PIX, boleto ou cartão de crédito)</li>
                  <li>- Para cartão de crédito, você será redirecionado ao ambiente seguro do Asaas</li>
                  <li>- Nenhum dado de cartão é armazenado por nós</li>
                </>
              ) : (
                <>
                  <li>- Os dados do seu cartão são processados pela Stripe</li>
                  <li>- Para compras online, recomenda-se o uso de cartão online/virtual</li>
                </>
              )}
            </ul>
      ...
      <div className="fixed inset-x-0 bottom-3 z-30 px-4 md:hidden">
        <div className="mx-auto flex max-w-4xl items-center justify-between gap-3 rounded-full border border-white/70 bg-white/92 px-4 py-3 shadow-lg backdrop-blur">
          <div className="min-w-0">
            ...
            <p className="text-sm font-semibold text-slate-900">Pagamento seguro</p>
          </div>
          <span className="text-xs text-slate-500">Etapa final</span>
        </div>
      </div>
